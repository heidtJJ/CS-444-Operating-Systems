...
ShareMemInit()
{
  ...
  // physical address: V2P ka_jared
  // page size: PGSIZE
  // permission U + W
  // call mappagesi
  // return va_jared
int
shm_jared(int key, int num_pages)
{
  // first see if this process has already allocated pages with this key
  for(int i = 0; i < 20; ++i){
  ...
  // first find if the key is used yet
  // if its not, then create it and update numPages
  int keyExists = -1;
  for(int i = 0; i < PAGES; ++i){
    if(keys[i] == key){
      keyExists = 1;
  ...
  int counter = 0;
  if(keyExists == -1){// must assign num_pages pages to the key, key does not exists currently
    for(int i = 0; i < PAGES; ++i){

      if(keys[i] == -1 && counter < num_pages){ 
        keys[i] = key;
        int allocatedPages = proc->allocatedPages; 
  ...
  else{// must include shared mem in the process
  ...
  // insert the key into keys used by process
  for(int i = 0; i < 20; ++i){
  ...
  return (int)(KERNBASE - 0x1000*proc->allocatedPages);
}
